...
var _ntes_domain_array = [ "163.com", "188.com", "netease.com", "126.com",
		"126.net", "nease.net", "yeah.net", "gz2010.cn", "co188.com",
		"warcraftchina.com", "youdao.com", "leihuo.net", "gzapg2010.cn",
		"126.am", "lofter.com", "lede.com", "diablo3.cc", "starcraft2.com.cn",
		"battlenet.com.cn", "wowchina.com", "hearthstone.com.cn",
		"heroesofthestorm.com.cn", "bobo.com", "yixin.im", "kaola.com",
		"diablo3.com.cn", "tottenhamhotspur.com", "temai163.com", "16163.com",
		"iaafbeijing2015.com", "playoverwatch.cn", "lmlc.com", "blizzard.cn",
		"yunyanzx.com", "immortalconquest.com", "onmyojigame.com",
		"onmyojigame.jp", "blizzardtv.cn" ];
...
var _ntes_page_data = {
	initTime : window.performance && window.performance.timing ? window.performance.timing.connectStart
			: +new Date(),
	getRunTime : function() {
	...
	getActionTime : function() {
		...
		if (a._actStart) {
			return Math
					.round((a.actionTime + +new Date() - a._actStart) / 1000)
		} else {
...
function neteaseTracker(l, a, m, k) {
	...
	if (_ntes_cookie_enabled == null) {
		...
		_ntes_cookie_enabled = (ntes_get_cookie("__ntes__test__cookies") == b) ? true
				: false;
		document.cookie = "__ntes__test__cookies=" + b + "; expires="
	...
	if (d == 1) {
		...
		if (h == -1) {
			if (_ntes_cookie_enabled) {
				...
				_ntes_nvfi = 1;
				ntes_set_cookie_long("_ntes_nnid", _ntes_nvid + ","
						+ (new Date).getTime())
			}
		} else {
			var o = h.indexOf(",");
		...
		var n = [ "_nacc=", e, "&_nvid=", _ntes_nvid, "&_nvtm=", _ntes_nvtm,
				"&_nvsf=", _ntes_nvsf, "&_nvfi=", _ntes_nvfi, "&_nlag=",
				_ntes_nlag, "&_nlmf=", _ntes_nlmf, "&_nres=", _ntes_nres,
				"&_nscd=", _ntes_nscd, "&_nstm=", _ntes_nstm, "&_nurl=",
				_ntes_nurl, "&_ntit=", _ntes_ntit, "&_nref=", _ntes_nref,
				"&_nfla=", _ntes_flsh, "&_nssn=", _ntes_nssn, "&_nxkey=",
				(b + "" + Math.random()).substring(6, 20), "&_end1" ].join("");
		_ntes_nvsf = 0;
		_ntes_sendInfo("base", _ntes_src_addr + "/ntes?" + n,
				neteaseTracker.callback);
		neteaseTracker.callback = null;
		if (1) {
			var j = /(?:iPhone|iPad|iPod|Android|Adr |Windows Phone)/i;
			if (_ntes_nacc != "analytics" && _ntes_nacc != "research"
					&& _ntes_nacc != "siteinfo"
					&& _ntes_nurl.indexOf("https") != 0
...
function ntes_survey_popup() {
	if (_ntes_surv == 0) {
		...
		a = a.length == 1 ? "0" + a : a;
		if (typeof (_ntes_nvid) == "undefined" || _ntes_nvid.length != 32
				|| _ntes_nvid.substr(30, 2) != a) {
			return
		}
		var b = "//research.163.com/survey/";
		b = b + "?_nacc=" + _ntes_nacc + "&_nvid=" + _ntes_nvid;
		window.open(b, "",
				"width=680, height=450, top=100, left=120, scrollbars=yes");
		ntes_set_cookie_long("_ntes_nnid", _ntes_nvid + ","
				+ (new Date).getTime())
	}
}
...
function fetch_visitor_hash() {
	...
	var a = str_to_ent(c.getTime() + Math.random() + document.location
			+ document.referrer + screen.width + screen.height
			+ navigator.userAgent + document.cookie + b);
...
function ntes_set_cookie_new(b, d, a) {
	...
	document.cookie = b + "=" + d + "; expires=" + c.toGMTString()
			+ "; path=/; domain=" + _ntes_cdmn
}
...
function ntes_page_click_stat(k) {
	...
	case "area":
		...
		q = this.innerText || this.textContent || this.title || this.alt
				|| this.name || "";
		if (m == "a" && !q) {
	...
	var r = escape(i), f = escape(q), d = escape(document.URL), j = document.compatMode === "CSS1Compat" ? document.documentElement.clientWidth
			: document.body.clientWidth, c = document.documentElement.scrollLeft
			|| document.body.scrollLeft || 0, a = document.documentElement.scrollTop
			|| document.body.scrollTop || 0, b = document.compatMode === "CSS1Compat" ? document.documentElement.clientHeight
			: document.body.clientHeight, l = _ntes_page_data.getRunTime(), s = Math
			.round(o.pageX), p = Math.round(o.pageY);
	var n = [ "_nacc=", _ntes_nacc, "&_npurl=", d, "&_nah=", r, "&_nat=", f,
			"&_ncw=", j, "&_nch=", b, "&_nsl=", c, "&_nst=", a, "&_nmx=", s,
			"&_nmy=", p, "&_nct=", l, "&_end" ].join("");
	_ntes_page_data.maxClick++;
	_ntes_sendInfo("pst", _ntes_src_addr + "/ntes_p?" + n)
}
function ntes_page_unload_stat(i) {
	var d = escape(document.URL), g = document.compatMode === "CSS1Compat" ? document.documentElement.clientWidth
			: document.body.clientWidth, f = document.documentElement.scrollLeft
			|| document.body.scrollLeft || 0, a = document.documentElement.scrollTop
			|| document.body.scrollTop || 0, b = document.compatMode === "CSS1Compat" ? document.documentElement.clientHeight
			: document.body.clientHeight, j = _ntes_page_data.getRunTime(), c = Math
			.max(_ntes_page_data.maxScroll, a), k = _ntes_page_data.maxClick, h = _ntes_page_data
			.getActionTime(), l = [ "_nacc=", _ntes_nacc, "&_npurl=", d,
			"&_ncw=", g, "&_nch=", b, "&_nsl=", f, "&_nst=", a, "&_nct=", j,
			"&_msl=", c, "&_mcn=", k, "&_nit=", h, "&_end" ].join("");
...
function neteaseClickStat(c) {
	...
	j = 0;
	for (; j < f; j++) {
		if (k[j].type == "button" || k[j].type == "submit"
				|| k[j].type == "img") {
	...
	_ntes_bindEvent(window, "scroll", e = function() {
		_ntes_page_data.maxScroll = Math.max(_ntes_page_data.maxScroll,
...
function _ntes_fixEvent(f) {
	var c = {}, g = "altKey ctrlKey shiftKey keyCode button clientX clientY offsetX offsetY pageX pageY target relatedTarget srcElement fromElement toElement"
			.split(" ");
	...
	if (c.pageX == null && c.clientX != null) {
		...
		c.pageX = c.clientX + (d && d.scrollLeft || a && a.scrollLeft || 0)
				- (d && d.clientLeft || a && a.clientLeft || 0);
		c.pageY = c.clientY + (d && d.scrollTop || a && a.scrollTop || 0)
...
function _ntes_sendInfo(c, b, f) {
	var e = c + "_" + (+new Date()) + parseInt(Math.random() * 100), a, d = f
			|| _ntes_void;
...
function recordAction(f, a, b, d, c) {
	...
	e += "&r=" + _ntes_nurl;
	e += "&_nxkey="
			+ (Number(new Date()) + "" + Math.random()).substring(6, 20)
			+ "&_end1";
...
function ntes_area_click_stat(v, k, b) {
	...
	case "area":
		...
		o = this.innerText || this.textContent || this.title || this.alt
				|| this.name || "";
		if (f == "a" && !o) {
	...
	var j = escape(u), c = escape(o), i = escape(b), d = s.width, m = s.height, w = k.scrollLeft || 0, r = k.scrollTop || 0, g = _ntes_page_data
			.getRunTime(), p = Math.round(q.pageX) - a.x + w, n = Math
			.round(q.pageY)
			- a.y + r;
	var l = [ "_nacc=", _ntes_nacc, "&_npurl=", i, "&_nah=", j, "&_nat=", c,
			"&_ncw=", d, "&_nch=", m, "&_nsl=", w, "&_nst=", r, "&_nmx=", p,
			"&_nmy=", n, "&_nct=", g, "&_end" ].join("");
...
var ntes_area_click_tools = {
	getOffset : function(d) {
		...
		var f = d.getBoundingClientRect(), j = d.ownerDocument, g = j.body, b = j.documentElement, e = b.clientTop
				|| g.clientTop || 0, h = b.clientLeft || g.clientLeft || 0, i = f.top
				+ (self.pageYOffset || this.boxModel && b.scrollTop || g.scrollTop)
				- e, c = f.left
				+ (self.pageXOffset || this.boxModel && b.scrollLeft || g.scrollLeft)
				- h;
		return {
			x : c,
			y : i
