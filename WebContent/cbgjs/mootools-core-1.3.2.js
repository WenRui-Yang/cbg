(function() {
	...
	force(
			...
			'Function', Function, [ 'apply', 'call', 'bind' ])('RegExp',
			RegExp, [ 'exec', 'test' ])(
			'Object',
			Object,
			[ 'create', 'defineProperty', 'defineProperties', 'keys',
					'getPrototypeOf', 'getOwnPropertyDescriptor',
					'getOwnPropertyNames', 'preventExtensions', 'isExtensible',
					'seal', 'isSealed', 'freeze', 'isFrozen' ])('Date', Date,
			[ 'now' ]);
	...
	this.$each = function(iterable, fn, bind) {
		...
		((type == 'arguments' || type == 'collection' || type == 'array' || type == 'elements') ? Array
				: Object).each(iterable, fn, bind);
	};
	this.$empty = function() {
	};
...
Function.implement({
	...
	bind : function(bind) {
		var self = this, args = (arguments.length > 1) ? Array.slice(arguments,
				1) : null;
		return function() {
...
Function.implement({
	create : function(options) {
		...
		return function(event) {
			...
			args = (args != null) ? Array.from(args) : Array.slice(arguments,
					(options.event) ? 1 : 0);
			if (options.event)
			...
			if (options.attempt)
				return Function.attempt(returns);
			return returns();
		};
	},
	...
	bindWithEvent : function(bind, args) {
		...
		if (args != null)
			args = Array.from(args);
		return function(event) {
			return self.apply(bind, (args == null) ? arguments : [ event ]
					.concat(args));
		};
	},
		...
		[ 'pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift',
				...
				.each(function(name) {
					...
					Array[name] = function(item) {
						return method.apply(Array.from(item), slice.call(
								arguments, 1));
					};
				});
	}
...
var Event = new Type(
		...
		function(event, win) {
			...
			if (type.indexOf('key') != -1) {
				...
				client = {
					x : (event.pageX != null) ? event.pageX - win.pageXOffset
							: event.clientX,
					y : (event.pageY != null) ? event.pageY - win.pageYOffset
							: event.clientY
				};
				if ((/DOMMouseScroll|mousewheel/).test(type)) {
					wheel = (event.wheelDelta) ? event.wheelDelta / 120
							: -(event.detail || 0) / 3;
				}
				...
				if ((/over|out/).test(type)) {
					...
					var hasRelated = (Browser.firefox2) ? testRelated.attempt()
							: testRelated();
...
(function() {
	...
	local.setDocument = function(document) {
		...
		features.contains = (root && this.isNativeCode(root.contains)) ? function(
				context, node) {
			return context.contains(node);
		}
				: (root && root.compareDocumentPosition) ? function(context,
						node) {
					return context === node
...
(function() {
	...
	var camels = [ 'defaultValue', 'accessKey', 'cellPadding', 'cellSpacing',
	...
	Element
			.implement({
				...
				getProperty : function(attribute) {
					...
					return (key) ? this[key]
							: (bools[attribute]) ? !!this[attribute]
									: (uriAttrs.test(attribute) ? this
											.getAttribute(attribute, 2)
											: (key = this
													.getAttributeNode(attribute)) ? key.nodeValue
													: null)
											|| null;
				},
				...
				getPrevious : function(expression) {
					return document.id(Slick.find(this, injectCombinator(
							expression, '!~')));
				},
				getAllPrevious : function(expression) {
					return Slick.search(this,
							injectCombinator(expression, '!~'), new Elements);
				},
				...
				getFirst : function(expression) {
					return document.id(Slick.search(this, injectCombinator(
							expression, '>'))[0]);
				},
	...
	var cleanClone = function(node, element, keepid) {
		...
		if (node.clearAttributes) {
			...
			if (node.options) {
				...
				for (var i = no.length; i--;)
					no[i].selected = eo[i].selected;
			}
		}
	...
	Element.implement('clone', function(contents, keepid) {
		...
		if (contents) {
			var ce = clone.getElementsByTagName('*'), te = this
					.getElementsByTagName('*');
			for (i = ce.length; i--;)
				cleanClone(ce[i], te[i], keepid);
		}
	...
	var setOpacity = function(element, opacity) {
		...
		if (hasOpacity) {
			...
			var filter = element.style.filter
					|| element.getComputedStyle('filter') || '';
			element.style.filter = reAlpha.test(filter) ? filter.replace(
					reAlpha, opacity) : filter + opacity;
		}
	};
	Element.Properties.opacity = {
		...
		get : (hasOpacity) ? function() {
			var opacity = this.style.opacity
					|| this.getComputedStyle('opacity');
			return (opacity == '') ? 1 : opacity;
		}
				: function() {
					var opacity, filter = (this.style.filter || this
							.getComputedStyle('filter'));
					if (filter)
						opacity = filter.match(reAlpha);
					return (opacity == null || filter == null) ? 1
							: (opacity[1] / 100);
				}
	};
	...
	Element
			.implement({
				...
				getStyle : function(property) {
					...
					if (Browser.opera
							|| (Browser.ie && isNaN(parseFloat(result)))) {
						...
						if (Browser.opera && String(result).indexOf('px') != -1)
							return result;
						if ((/^border(.+)Width|margin|padding/).test(property))
							return '0px';
					}
					return result;
				},
	...
	var delegation = {
		addEvent : function(type, match, fn) {
			...
			if (_map.condition || elementEvent && elementEvent.condition) {
				var __match = match, condition = _map.condition
						|| elementEvent.condition;
...
(function(win) {
	...
	Element
			.implement({
				$attr : function(name, value) {
					if ($type(name) == 'object')
						var ret = this.setProperties(name);
					var ret = (!value) ? this.getProperty(name) : this
				...
				$val : function(value) {
					var ret = (!value) ? this.getProperty("value") : this
				...
				$width : function(width) {
					...
					[ 'padding-left', 'padding-right', 'border-left-width',
							'border-right-width' ].each(function(attr) {
				...
				$height : function(height) {
					...
					[ 'padding-top', 'padding-bottom', 'border-top-width',
							'border-bottom-width' ].each(function(attr) {
						var value = parseInt(ctx.getStyle(attr));
				...
				$position : function() {
					var offset = this.getOffsets(), ptOffset = this
							.getOffsetParent().getOffsets();
					return {
						top : offset.y - ptOffset.y,
						left : offset.x - ptOffset.x
					};
				},
				$offset : function() {
	...
	(function() {
		var protoAnimate = {
			$animate : function(css, duration, callback, fn) {
				var ctx = this;
				var animations = ctx.$animations = ctx.$animations
						|| new AnimateList(ctx);
				if ($type(css) === 'object') {
		...
		var Color = {
			...
			isColor : function(value) {
				value = value + '';
				return !!(value.match(/^#[0-9a-f]{3,6}$/i) || value
						.match(/(\d+),\s*(\d+),\s*(\d+)/));
			}
		};
		var BaseTime = 20;
		function Animate(elem, options) {
			...
			ctx.css = options.css || {};
			ctx.fn = options.fn
					|| function(result, currentTime, beginValue, distanceValue,
							totalTime) {
						return beginValue + distanceValue * currentTime
								/ totalTime;
					};
			ctx.oldCss = {};
			ctx.isRunning = false;
	...
	(function() {
		...
		Request.JSON = new Class({
			Extends : Request._JSON,
			initialize : function(options) {
				this.parent(options);
				this.addEvent('error', function(text, error) {
					try {
						sendLog({
							"msg" : "JSON_REQUEST_PARSE",
							"info" : (text || "").trim().substring(0, 50),
			...
			onSuccess : function(json, text) {
				try {
					if (json.status == AjaxConstants.Error) {
						sendLog({
							"msg" : "JSON_RESPONSE_ERROR",
							"info" : text,
							"target" : this.options.url
						});
					}
